...
protected   
  parameter String inputFileName=Modelica.Utilities.Files.loadResource("{{inputFilePath}}") 
    "Name of the CYMDIST input file";
  parameter Integer resWri={{writeResults}} 
  ...
  {% set comma = joiner(",") -%} 
  Real yR[nDblOut]={
  {%- for row in concatOutputVariableNames -%}
  {{comma()}}
  '{{row}}'
  {%- endfor %} 
  }"Variables used to collect values received from CYMDIST";
