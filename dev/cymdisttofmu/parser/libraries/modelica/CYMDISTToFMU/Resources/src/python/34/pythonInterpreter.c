...
The error message is \"%s\"",
  ...
  if ( nStrWri > 0 ){
    ...
    for (i = 0; i < nStrWri; ++i) {
      ...
      /*      pValue = PyUnicode_FromStringAndSize(strValWri[i], len);*/
      pValue = PyUnicode_FromString(strValWri[i]);
      if (!pValue) {
	/* Failed to convert argument.*/
	Py_DECREF(pArgsStr);
	Py_DECREF(pModule);
...
The returned object is \"%s\".",
				functionName, PyBytes_AsString(PyObject_Repr(pValue)));
	  intValRea[pIndVal] = PyLong_AsLong( p );
	} /* for(...)*/
...
The error message is \"%s\"",
  ...
  pValue = PyUnicode_FromString(inputFileName);
  if (!pValue) {
	  /* Failed to convert argument.*/
	  Py_DECREF(pArgsStr);
	  Py_DECREF(pModule);
