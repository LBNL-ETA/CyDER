...
This section shows how to export a Python script as an FMU.
The FMU FMU has one input ``v`` and one output ``i``.
The python script receives the input and calculate the output
i as i=v/R where R is a parameter defined in the Python script.

First, to export the Python script the user needs to create a Simulator XML input file.
The following snippet shows auch an input file

.. literalinclude:: ../../../parser/utilities/SimulatorModelDescription.xml
   :language: xml
   :linenos:

Second, the user needs to write the Python script which will be exported as an FMU.
The following snippet shows auch a python script

.. literalinclude:: ../../../parser/utilities/example.py
   :language: xml
   :linenos:
