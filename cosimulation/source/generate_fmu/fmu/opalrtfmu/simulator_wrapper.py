...
def compileAndInstantiate(projectPath):
    ...
    try:
        ...
        if modelState == RtlabApi.MODEL_RUNNING or modelState == RtlabApi.MODEL_PAUSED:
            ...
            ## Pause the model
            log.info ("=====The model is running or paused and won't be recompiled.")
            RtlabApi.Disconnect()
            return -11
...
def setData(projectPath, inputNames, inputValues, simulationTime):
    ...
    try:
        ...
        log.info ("=====The simulation stoptime={!s}.".format(stopTime))
...
def convertUnicodeString(inputNames):
    ...
    """
    This function gets an unicode string and converts it to a string.
...
def exchange(projectPath, simulationTime, inputNames, inputValues, outputNames, writeResults):
    ...
    #projectName = os.path.abspath(os.path.join('examples', 'demo', 'demo.llp'))
     ...
     if (retVal <> -11):
        log.info ("=====Return if the model hasn't been compiled yet.")
        return
     ...
     if (outputNames is not None):
         ...
         if(outputValues is None):
             log.error ("output values for outputNames={!=} is empty at time={!s}.".
    ...
    #  retOutputValues = []
    #  log.info("The outputValues are={!s}".format(outputValues))
